            ...
            if parts[0].lower() == 'basic' and len(parts) >= 2:
                ...
                if len(auth) >= 2:
                    ...
                    if user is not None:
                        login(request, user)
                        log.info(
                            'BasicAuthMiddleware: success...logged in %s', user.get_full_name)
                    else:
                        log.info(
                            'BasicAuthMiddleware: failed...email or password did not match')
                        raise PermissionDenied(
                            'Invalid email or password in HTTP Basic Authentication')
